...
type RosenMemberVO struct {
	...
	StarSign              string   `json:"constellation"`
	Wants                 []string `json:"wants,omitempty"`
	Offers                []string `json:"offers,omitempty"`
	Personality           string   `json:"mbti"`
...
func (c *Controller) composeRosenMemberFullVO(ext *MemberExtra, sns *member.SnsSummary, energy *account.Account, gem *account.Account, rosenCoin *account.Account, snsId *member.SnsID) *MemberFullVO {
	...
	if strings.Contains(m.Avatar, "http") {
		...
		if avatarURL, err := c.OssController.OssDownloadURLWithoutPrefix(m.Avatar); err != nil {
	...
	vo := &MemberFullVO{
		...
		Is3rdParty:            len(m.LoginPwd) == 0,
		Area:                  ext.Area,
		StarSign:              ext.StarSign,
		Personality:           ext.Personality,
	}
	vo.Privileges = []string{}
	for i := range ext.Privileges {
		vo.Privileges = append(vo.Privileges, ext.Privileges[i].Name)
	}
	if snsId != nil {
		vo.Platform = snsId.SnsType
	}
	if sns != nil {
	...
	if len(ext.Wallets) > 0 {
		cdevcfg := config.GetConfig().Rosen.CircleService
		if cdevcfg.Enable {
			hasCircleWallet := false
			// 查找用户是否使用了circle钱包
			for _, w := range ext.Wallets {
				if w.Chain == "CircleDevWalletWeb3Service" {
			...
			if hasCircleWallet {
				// 找到了，去掉系统solana钱包
				for _, w := range ext.Wallets {
					if len(w.PriKey) == 0 || w.Chain != "solana" {
						wallets = append(wallets, w)
					}
					if w.Chain == "CircleDevWalletWeb3Service" && len(w.PriKey) > 0 {
						w.Chain = "solana"
						wallets = append(wallets, w)
					}
				}
	...
	if gem != nil {
		vo.Gem = float64(gem.Available) / math.Pow10(int(config.GetConfig().Rosen.Coin2.Decimals))
	}
	if rosenCoin != nil {
...
type SnsFriendRequestVo struct {
	...
	Sender   *MemberWithEquipVO `json:"sender,omitempty"`
	Receiver *MemberWithEquipVO `json:"receiver,omitempty"`
}
