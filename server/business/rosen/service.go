...
import (
	...
	"github.com/GoROSEN/go-circle-web3-service/notifications"
...
type Service struct {
	...
	cdevWalletService *devwallets.CircleDevWalletsService
}

// NewService 新建服务
func NewService(_db *gorm.DB, rds *redis.Client, account *account.AccountService, geoip *geoip2.Reader, msgMod *message.MsgMod, cdevWallet *devwallets.CircleDevWalletsService) *Service {
...
func (s *Service) SetupNewMember(db *gorm.DB, obj *member.Member) error {
	...
	for i := range chains {
		...
		} else if c.Name == "bnb" || c.Name == "okc" || c.Name == "polygon" || c.Name == "ethereum" {
		...
		if bc != nil {
			...
			for _, token := range c.Tokens {
				log.Infof("create member token wallet for %v/%v", token.Name, c.Name)
				var tokenAcc string
				if token.AutoCreate {
					_, err := bc.NewTokenAccount(token.ContractAddress, pub)
					if err != nil {
						log.Errorf("cannot create ata account: %v", err)
						return err
					}
				}
				tokenAcc, _ = bc.FindTokenAccount(token.ContractAddress, pub)
	...
	//TODO：增加circle dev wallet
	if s.cdevWalletService != nil {
		cfg := config.GetConfig().Rosen.CircleService
		if cdw, err := s.cdevWalletService.CreateWallet(cfg.DevWallet.WalletSetId, cfg.DevWallet.Type, cfg.DevWallet.BlockChains, 1); err != nil {
			log.Errorf("cannot create wallet on Circle Dev Wallet: %v", err)
		} else if len(cdw) > 0 {

			wallet := &Wallet{
				OwnerID: obj.ID,
				Chain:   "CircleDevWalletWeb3Service",
				Address: cdw[0].Address,
				PubKey:  cdw[0].Address,
				PriKey:  cdw[0].ID, // walletId
			}
			if err := db.Create(&wallet).Error; err != nil {
				log.Errorf("save CircleDevWallet error: %v", err)
				// return err
			}

			for _, token := range cfg.DevWallet.Tokens {
				wallet = &Wallet{
					OwnerID:         obj.ID,
					Chain:           "CircleDevWalletWeb3Service",
					Token:           token.Name,
					ContractAddress: token.ID, // tokenId
					Address:         cdw[0].Address,
					PubKey:          cdw[0].Address,
					PriKey:          cdw[0].ID, // walletId
				}
				if err := db.Create(&wallet).Error; err != nil {
					log.Errorf("save token of CircleDevWallet error: %v", err)
					// return err
				}
			}
		} else {
			log.Errorf("created 0 wallet on Circle Dev Wallet!!")
		}
	}
	return nil
}
...
func (s *Service) MintNFT(chain string, extra *MemberExtra, plot *Plot, count int, imgOssUrl, imgIpfsUrl string, producerAcc *account.Account) (float64, error) {
	...
	enableCircle := config.GetConfig().Rosen.CircleService.Enable
	for _, wa := range extra.Wallets {
		if enableCircle {
			if (wa.Chain == chain ||
				(chain == "solana" && wa.Chain == "CircleDevWalletWeb3Service")) &&
				(len(toAddress) == 0 || len(wa.PriKey) == 0) {
				// 用户自有钱包优先
	...
	if len(plot.MintPriceByChains) > 0 {
...
func (s *Service) WithdrawToken(memberId uint, amount float64, token, chain string) error {
	...
	var tokenCfg *config.BlockChainTokenConfig
	...
	for _, tcfg := range chainCfg.Tokens {
		if tcfg.Name == token {
			tokenCfg = tcfg
			break
		}
	...
	if tokenCfg == nil {
		log.Errorf("invalid token type: %v", token)
		return errors.New("message.common.system-error")
	}

	acc := s.accountService.GetAccountByUserAndType(memberId, tokenCfg.AccountType)
	if acc == nil {
	...
	if chainAccess != nil {
		r := big.NewInt(0)
		if tokenCfg.Decimals >= decimals {
			r.Mul(big.NewInt(amount0), big.NewInt(int64(math.Pow10(int(tokenCfg.Decimals-decimals)))))
			tx, err = chainAccess.TransferToken(funder, wallet.Address, r, tokenCfg.ContractAddress, tokenCfg.Decimals)
		} else {
	...
	if err != nil {
		...
		// verify tx
		if confirmed, err := chainAccess.ConfirmTransaction(tx); err != nil {
			// 连不上链
			log.Errorf("cannot confirm tx(%v): %v", tx, err)
		} else if !confirmed {
			// 无法确认
			log.Errorf("tx(%v) confirmation failed, try it later", tx)
		}
		// commit
...
func (s *Service) WithdrawTokenV2(memberId uint, amount float64, token, chain string) error {
	...
	if err := s.CreateModel(&request); err != nil {
		s.accountService.Unfreeze(acc, acc, amount0, "create withdraw request failed")
		log.Errorf("cannot create request for withdraw: %v", err)
		return errors.New("message.common.system-error")
	}
...
func (s *Service) ApproveWithdrawToken(memberId uint, amount float64, token, chain string) error {
	...
	if wallet.Chain == "solana" {
		chainAccess, err = blockchain.NewSolanaChainAccess(chainCfg)
	} else if wallet.Chain == "bnb" || wallet.Chain == "okc" || wallet.Chain == "polygon" || wallet.Chain == "ethereum" {
		chainAccess, err = blockchain.NewGethChainAccess(chainCfg)
	}
	if chainAccess != nil {
		r := big.NewInt(0)
...
func (s *Service) RejectWithdrawToken(memberId uint, amount float64, token, chain string) error {
	...
	s.accountService.Unfreeze(acc, acc, amount0, "withdraw failed on chain")
	log.Infof("rejected withdraw request")
...
func (s *Service) MintSuitNFT(chain string, extra *MemberExtra, suit *Asset, imgIpfsUrl string) error {
	...
	var transferrable bool
	for _, wa := range extra.Wallets {
		if (wa.Chain == chain || (chain == "solana" && wa.Chain == "CircleDevWalletWeb3Service")) &&
			(len(toAddress) == 0 || len(wa.PriKey) == 0) {
			// 用户自有钱包优先
			toAddress = wa.Address
			transferrable = len(wa.PriKey) > 0
...
func (s *Service) InAppPurchase(channel, transactionId, productId, currency string, quantity, price int64, memberId uint, transaction interface{}) error {

	coinCfg := &config.GetConfig().Rosen.Coin2
	var goldAmount int64 = 0
	decimals := int64(coinCfg.Decimals)
	t := int64(math.Pow10(int(decimals))) * quantity
	switch productId {
	// new
	case "gems_100":
		goldAmount = 1 * t
	...
	if goldAmount > 0 {
		// 确认收据
		if err := s.accountService.SaveReceiption(channel, transactionId, memberId, transaction); err != nil {
			return fmt.Errorf("cannot confirm transaction(%v): %v", transactionId, err)
		}
		// 加钱
		acc := s.accountService.Account(uint(memberId), coinCfg.TokenName)
		if acc == nil {
			return fmt.Errorf("cannot find account for %v@%v", memberId, coinCfg.TokenName)
		}
		s.accountService.IncreaseAvailable(acc, goldAmount, fmt.Sprintf("charge from apple IAP, txid = %v, currenct = %v, quantant = %v, price = %v", transactionId, currency, quantity, price))
	} else {
		return fmt.Errorf("invalid product id: %v", productId)
	}
	return nil
}

func (s *Service) ChargeMemberToken(memberId uint, token string, amount float64, decimals int, reason string) error {
	// check member is valid
	var m MemberExtra
	if err := s.GetPreloadModelByID(&m, uint(memberId), []string{"Member"}); err != nil {
		log.Errorf("ChargeMemberToken: cannot find member: %v", err)
		return err
	}
	// transfer token
	toAccount := s.accountService.GetAccountByUserAndType(m.MemberID, token)
	if decimals > 0 {
		if _, err := s.accountService.IncreaseAvailable(toAccount, int64(amount*math.Pow10(decimals)), reason); err != nil {
			log.Errorf("ChargeMemberToken: Transfer failed: %v", err)
			return err
		}
	} else {
		if _, err := s.accountService.IncreaseAvailable(toAccount, int64(amount), reason); err != nil {
			log.Errorf("ChargeMemberToken: Transfer failed: %v", err)
			return err
		}
	}
...
func (s *Service) AirdropMemberToken(memberId uint, token string, amount float64, decimals int, reason string, sendNotify bool) error {
	// check member is valid
	var m MemberExtra
	if err := s.GetPreloadModelByID(&m, uint(memberId), []string{"Member"}); err != nil {
		log.Errorf("AirdropMemberToken: cannot find member: %v", err)
		return err
	}
	// transfer token
	sysAccount := s.accountService.GetAccountByUserAndType(kSystemAccountID, token)
	toAccount := s.accountService.GetAccountByUserAndType(m.MemberID, token)
	if sysAccount == nil || toAccount == nil {
		log.Errorf("AirdropMemberToken: cannot find account")
		return errors.New("cannot find account")
	}
	if decimals > 0 {
		if _, err := s.accountService.Transfer(sysAccount, toAccount, int64(amount*math.Pow10(decimals)), reason); err != nil {
			log.Errorf("AirdropMemberToken: Transfer failed: %v", err)
			return err
		}
	} else {
		if _, err := s.accountService.Transfer(sysAccount, toAccount, int64(amount), reason); err != nil {
			log.Errorf("AirdropMemberToken: Transfer failed: %v", err)
			return err
		}
	}
	...
	if sendNotify {
		...
		s.msgMod.SendMessage(m.MemberID, "info-airdrop-token", m.Member.Language, params)
	}
	return nil
}

func (s *Service) VerifyCircleNotificationSignature(signature, keyid string) bool {

	cfg := config.GetConfig().Rosen.CircleService
	if !cfg.Enable {
		log.Infof("circle service is disabled")
		return false
	}

	ns := notifications.NewNotificationService(cfg.Host, cfg.Apikey)
	pubkey, err := ns.GetPublicKey(keyid)
	if err != nil {
		log.Errorf("cannot get pubkey: %v", err)
		return false
	}
	log.Infof("got pubkey: %v", pubkey)
